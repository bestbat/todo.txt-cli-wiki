...
** "Add with created-on date":#add-with-created-on
...
** "painless-recurring - Who needs an add-on script? Just uses aliases!":#painless-recurring
...
h2(#add-with-created-on). Add with created-on date
...
h2(#stale--prune-stale-todos). stale - Move stale todos into a stale.txt file
...
h2(#remind--create-apple-reminders). remind - Create Apple Reminders out of your todos (Mac only)
...
h2(#book--create-calendar-events). book - Create calendar events out of your todos
...
h2(#setdue--set-duedates). setdue - Set or modify the due-date of one or several already-existing items
...
h2(#many). many - make the same change on multiple tasks
...
h2(#hamster-clock). Clock: Track time for tasks through GNOME hamster
...
h2(#random). random -- choose a task randomly
...
h2(#today--see-todays-tasks). today - Display tasks tagged for today
...
h2(#autoprioritise-tasks). Autoprioritise tasks
...
h2(#painless-recurring). Painless-recurring tasks

Use add-and-do-in-one-step plus aliases

alias tx="t add x"
alias ta="t archive"

"Project Page":https://github.com/vcg3rd/todo.txt-painless-recurring/blob/main/README.md