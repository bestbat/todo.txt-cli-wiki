...
This action provides a shorthand for moving todos into and out of a stale.txt file, where you would keep todos that you haven't got to in a while and that don't really fit into a specific outline. In the future, it will also be able to automatically prune todos that are older than a certain threshold (if they have a creation date).
...
This action creates an Apple Reminder out of a todo. It also includes an optional customized do action that will find the matching Apple Reminder and delete it once you mark the todo as done. In the future, it will also have a "round trip" action that will read your completed Reminders and complete the corresponding todos.
